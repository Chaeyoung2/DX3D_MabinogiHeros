#include "stdafx.h"
#include "..\Headers\Gaurd_UI.h"
#include "Component_Manager.h"
#include "Object_Manager.h"
#include "Shader.h"

CGaurd_UI::CGaurd_UI(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CGaurd_UI::Ready_GameObject(_int iIndex)
{
...
_int CGaurd_UI::Update_GameObject(const _float & fTimeDelta)
{
...
_int CGaurd_UI::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CGaurd_UI::Render_GameObject(void)
{
...
HRESULT CGaurd_UI::Ready_Component(void)
{
	...
	// For.Com_Texture
		...
		pComponent = m_pTextureCom = (CTexture*)m_pComponent_Manager->Clone_Component(SCENE_STAGE, L"Component_Texture_UI_Gaurd_0");
...
HRESULT CGaurd_UI::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CGaurd_UI * CGaurd_UI::Create(LPDIRECT3DDEVICE9 pGraphicDev, _int iIndex)
{
	CGaurd_UI *	pInstance = new CGaurd_UI(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject(iIndex)))
	{
		MSG_BOX("CGaurd_UI Created Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CGaurd_UI::Free(void)
{
