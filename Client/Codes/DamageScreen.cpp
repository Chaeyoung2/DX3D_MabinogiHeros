#include "stdafx.h"
#include "..\Headers\DamageScreen.h"
#include "Component_Manager.h"
#include "Object_Manager.h"
#include "Shader.h"

CDamageScreen::CDamageScreen(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CDamageScreen::Ready_GameObject(void)
{
...
_int CDamageScreen::Update_GameObject(const _float & fTimeDelta)
{
...
//  	system("cls");
//  	cout << "m_fAlphaValue : " << m_fAlphaValue << endl;
// 	cout << "m_bIsAlphaValueUp : " << m_bIsAlphaValueUp << endl;
	m_pRendererCom->Add_RenderGroup(Engine::CRenderer::RENDER_UI, this);

	m_fCreateTime += fTimeDelta;

	if (m_bIsAlphaValueUp == false)
	{
	...
	if (m_fAlphaValue >= 0.9f)
	{
		m_bIsAlphaValueUp = false;
	}
	if (m_fAlphaValue <= 0.3f)
	{
		m_bIsAlphaValueUp = true;
	}
...
_int CDamageScreen::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CDamageScreen::Render_GameObject(void)
{
...
HRESULT CDamageScreen::Ready_Component(void)
{
	...
	// For.Com_Texture
	pComponent = m_pTextureCom = (CTexture*)m_pComponent_Manager->Clone_Component(SCENE_STAGE, L"Component_Texture_UI_DamageScreen");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CDamageScreen::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CDamageScreen * CDamageScreen::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CDamageScreen *	pInstance = new CDamageScreen(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MSG_BOX("CDamageScreen Created Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CDamageScreen::Free(void)
{
