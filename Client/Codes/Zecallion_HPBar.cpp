#include "stdafx.h"
#include "..\Headers\Zecallion_HPBar.h"
#include "Component_Manager.h"
#include "Object_Manager.h"
#include "Shader.h"

CZecallion_HPBar::CZecallion_HPBar(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CZecallion_HPBar::Ready_GameObject(void)
{
...
_int CZecallion_HPBar::Update_GameObject(const _float & fTimeDelta)
{
...
_int CZecallion_HPBar::LastUpdate_GameObject(const _float & fTimeDelta)
{
...
void CZecallion_HPBar::Render_GameObject(void)
{
...
HRESULT CZecallion_HPBar::Ready_Component(void)
{
	...
	// For.Com_Texture
	pComponent = m_pTextureCom = (CTexture*)m_pComponent_Manager->Clone_Component(SCENE_STAGE, L"Component_Texture_Zecallion_HPBar1");
	if (nullptr == pComponent)
		return E_FAIL;
...
HRESULT CZecallion_HPBar::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
CZecallion_HPBar * CZecallion_HPBar::Create(LPDIRECT3DDEVICE9 pGraphicDev)
{
	CZecallion_HPBar *	pInstance = new CZecallion_HPBar(pGraphicDev);

	if (FAILED(pInstance->Ready_GameObject()))
	{
		MSG_BOX("CZecallion_HPBar Created Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

_ulong CZecallion_HPBar::Free(void)
{
