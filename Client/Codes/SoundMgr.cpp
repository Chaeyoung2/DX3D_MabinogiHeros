...
IMPLEMENT_SINGLETON_NOTBASE(CSoundMgr)
...
void CSoundMgr::PlaySound( TCHAR* pSoundKey, CHANNEL_TYPE eChannel )
{
	map<TCHAR*, FMOD_SOUND*>::iterator iter = find_if(m_MapSound.begin(),
		m_MapSound.end(), CFind_Tag(pSoundKey));
...
void CSoundMgr::OncePlaySound( TCHAR* pSoundKey, CHANNEL_TYPE eChannel, float fVolume )
{
	map<TCHAR*, FMOD_SOUND*>::iterator iter = find_if(m_MapSound.begin(),
		m_MapSound.end(), CFind_Tag(pSoundKey));
